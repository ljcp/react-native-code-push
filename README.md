This is a fork on the original react-native-code-push with the only change that it only works for iOS , Android stuff is removed as at this point it's not compatible with some libs (see code push issues for examples)
